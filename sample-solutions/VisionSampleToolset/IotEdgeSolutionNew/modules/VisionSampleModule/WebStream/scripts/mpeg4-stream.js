...
class Mpeg4Stream {
    ...
    startVideo() {
        if (!this.isVideoStreaming()) {
            const ffmpegParams = `-loglevel fatal -i rtsp://${process.env.CAMERA_IP}:8900/live -vcodec copy -an -sn -dn -reset_timestamps 1 -movflags empty_moov+default_base_moof+frag_keyframe -bufsize 256k -f mp4 -seekable 0 -headers Access-Control-Allow-Origin:* -content_type video/mp4 http://127.0.0.1:${this.streamingPort}/${this.secret}`;
    ...
    startStreamingServer() {
        ...
        console.log(`Listening for incoming mp4 stream on http://localhost:${this.streamingPort}/${this.secret}`);
        console.log(`Awaiting WebSocket connections on ws://localhost:${this.wsPort}`);
    }
}
